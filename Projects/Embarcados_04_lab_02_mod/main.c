//*****************************************************************************
//
//	Lab02_HelloLaunchpad - modified version - launchpad Tiva/Stellaris
...
int main(void)
{
    ...
    while(1)
    {
    	...
    	In = GPIOPinRead(GPIO_PORTF_BASE, SW_1 | SW_2);

    	if (In == ((SW_1 | SW_2) & ~SW_1))
    	{
    		// set the green LED pin high, others low
        	GPIOPinWrite(GPIO_PORTF_BASE, LED_RED|LED_BLUE|LED_GREEN, LED_GREEN);	// set led Green
		}
    	else if (In == ((SW_1 | SW_2) & ~SW_2))
    	{
    		GPIOPinWrite(GPIO_PORTF_BASE, LED_RED|LED_BLUE|LED_GREEN, LED_RED|LED_GREEN|LED_BLUE);	// set led Green
    	}
    	else
    	{
